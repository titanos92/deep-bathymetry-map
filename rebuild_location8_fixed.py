...
TILES_DIR = Path(
    r"C:\Users\tytar\tiles_location8"
)

BASE_DIR = Path(__file__).resolve().parent

POINTS_FILE = (
    BASE_DIR / "depth_points_location8_fixed.csv"
)

POLYGONS_FILE = (
    BASE_DIR / "depth_polygons_location8_fixed.geojson"
)
...
def tile_coordinate_to_lonlat(
    ...
    global_x = (
        tile_x
        +
        float(px) / float(extent)
    )

    global_y = (
        tile_y
        +
        float(py) / float(extent)
    )
...
def main():
    ...
    print(
        "REBUILD LOCATION 8 - CORRECT DEPTH SCALE"
    )
    ...
    print()
    print("IMPORTANT:")
    print("No HTTP requests will be made.")
    print(
        "Working river_* files will NOT be changed."
    )
    print(
        "deepmap backups will NOT be changed."
    )
    ...
    print(
        "LOCATION 8 FIXED REBUILD COMPLETE"
    )
